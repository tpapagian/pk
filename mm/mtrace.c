			      ...
			      gfp_t gfp_flags)
{
	...
	if (cachep)
		cachename = cachep->name;
	mtrace_label_register(mtrace_label_heap, ptr, bytes_alloc, 
			      cachename, strlen(cachename), call_site);
}
...
static void mtrace_mm_page_free(void *unused,
			        struct page *page,
			        unsigned int order)
{
    void * va = page_address(page);    
...
static void mtrace_mm_page_free_batched(void *unused,
				   	struct page *page,
				   	int cold)
{
    /* 
     * free_hot_cold_page_list ends up calling free_pages_prepare 
     * free_pages_prepare calls trace_mm_page_free
     */
}

 ...
 * struct stat_data systemtap_stat;
 *
 * histogram.type = HIST_LINEAR;
 * histogram.start = 0;
 * histogram.stop = 64;
 * histogram.interval = 1;
 * histogram.buckets = _stp_stat_calc_buckets(64, 0, 1);
 *
 * To add value to histogram:
 * _stp_stat_add(&histogram, &systemtap_stat, 1);
 *
 * To print the histogram:
 *
 * _stp_stat_print_histogram_buf(&histogram, &systemtap_stat);
 * memset(&systemtap_stat, 0, sizeof(struct stat_data));
*/
...
static void __stp_stat_print_histogram_seq(struct Hist *st, struct stat_data *sd, struct seq_file *f)
{
...
#define HIST_PRINTF(fmt, args...) \
	(f ? seq_printf(f, fmt, ## args) : printk(fmt, ## args))
